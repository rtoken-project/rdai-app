<template>
  <v-container text-sm-center py-5 mx-xs-auto fluid fill-height pa-0>
    <v-sheet elevation="3" class="pa-3 mx-auto" max-width="640">
      <v-layout
        grid-list-sm
        :class="{
      ...
      >
        <v-flex class="text-sm-center title my-auto pr-1 py-3 pb-5"
          >Select Beneficiaries</v-flex
        >
      </v-layout>
      ...
      <v-flex xs12 py-3>
        <web3-btn
          action="createHat"
          :disabled="
            addCommission ? hatInCreation.length < 1 : hatInCreation < 2
          "
          :params="{ switchToThisHat }"
          @then="goToCustom"
          >Create new Pool</web3-btn
        >
      </v-flex>
    </v-sheet>
    <v-dialog v-model="commissionModal" persistent max-width="340">
      <v-card class="pa-4">
        ...
        <v-card-text class="mt-4">
          <p>
            The 5% donation to rDai DAO is equal to only
            {{ (rate / 20) | formatNumber(2) }}% a year.
          </p>
          <p>
            While this might be a small change for you, it is the only way we
            can fund continued development of rDAI.
          </p>
          <p>
            If you still want to remove it, please consider donating to our DAO
            directly.
          </p>
          <p>
            Check out how we use funds and make a donation through our
            <a
              href="https://mainnet.aragon.org/#/rdao/0xc281d4fe88701bf30326b3bcb2c3eb73669d3b91"
              target="_blank"
              >Aragon DAO.</a
            >
          </p>
...
export default {
  ...
  computed: {
    ...
    commissionLabel() {
      return this.addCommission
        ? "Donating 5% of the interest to the rDAI dev DAO"
        : "Keeping all of the interest";
    },
    rate() {
  ...
  watch: {
    ...
    hatInCreation: {
      handler: function(newVal) {
        if (this.addCommission) {
          ...
          const hat = newVal;
          hat.proportions[0] =
            (newVal.totalProportions - newVal.proportions[0]) / 19;
    ...
    addCommission(newVal, oldVal) {
      if (!newVal) {
        if (
          this.hatInCreation.recipients[0].toLowerCase() ===
          featured[0].address.toLowerCase()
        )
          this.removeRecipient(0);
      } else {
  ...
  methods: {
    ...
    async addRecipient() {
      ...
      if ((await web3.eth.getCode(this.newAddress)) === "0x")
        this.localAlerts.push(false);
