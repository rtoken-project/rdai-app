<template>
  <v-container ref="container">
    <v-flex xs12 class="force-line">
      ...
      <v-btn v-if="qr" class="flex ma-0" icon flat @click.stop="qrCode(link)">
...
export default {
  ...
  methods: {
    ...
    doCopy: function() {
      ...
      this.$copyText(this.link, this.$refs.container).then(
        e => {
        ...
        e => {
          this.$store.commit("error", {
            text: "Error copying text. Please copy manually",
            icon: "mdi-link-off"
          });
        }
      );
