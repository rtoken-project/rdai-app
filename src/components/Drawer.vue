    ...
    <v-layout column>
        ...
        <v-list-item :key="i" :ripple="false">
          ...
          <v-list-item-content>
            <v-list-item-title class="text-xs-justify">
              {{ item.text
              }}<strong> {{ item.balance | formatNumber(4) }}</strong>
            </v-list-item-title>
          </v-list-item-content>
      ...
      <template v-if="userHat && userHat.recipients.length > 0">
        ...
        <v-list-item>
          ...
          <v-list-item-action
            v-if="$route.name !== 'interest'"
            class="pointer"
            @click.stop="$router.push('/interest')"
          >
...
export default {
    ...
    editedTxList() {
      const etherscan =
        this.chainName === "Mainnet"
          ? "https://etherscan.io"
          : `https://${this.chainName.toLowerCase()}.etherscan.io`;
      ...
      return this.txList.map(i => {
        ...
        switch (i.type) {
          ...
          case "changeHat":
            if (found.hasOwnProperty("shortTitle"))
              i.text = `Switch to ${found.shortTitle} pool`;
  ...
  methods: {
    showUserHat() {
      ...
      if (this.userHat.hasOwnProperty("shortTitle"))
        this.$router.push(`/donate/${this.userHat.shortTitle}`);
