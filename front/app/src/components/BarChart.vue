<template lang="html">
  <v-flex xs12 mt-3>
    ...
    <v-flex v-if="showCommission" class="caption text-left ml-1"
      ><v-icon small>fa fa-arrow-up</v-icon>&nbsp;&nbsp;5% is directed to the
      rDAI dev DAO</v-flex
    >
  </v-flex>
</template>
    ...
    proportions() {
      ...
      const p = proportions.map((i, index) => {
        ...
        if (this.hat.hasOwnProperty("colors")) {
          ...
          const inFeatured = featured.filter(
            b => b.address === recipients[index]
          );
