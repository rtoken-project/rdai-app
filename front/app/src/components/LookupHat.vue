<template>
  <v-layout align-center wrap pt-5 mt-5
    fluid
    fill-height
    pa-0
    >
    ...
    <v-flex sm8 shrink mx-auto text-xs-center>
      <v-text-field
        ...
        <template slot="append">
          <div v-if="lookupAddress.length===0 && hasWeb3"
    ...
    <v-flex xs12 mb-4  text-center  shrink text-center>
      ...
      <app-custom-hat v-if="typeof foundHat!=='undefined'" :hat="foundHat"></app-custom-hat>
    </v-flex>
  </v-layout>
</template>
...
export default {
  ...
  methods: {
      async getHatByAddressClicked(){
          ...
          this.hatByAddress = await this.$store.dispatch("getHatByAddress", this.lookupAddress);
